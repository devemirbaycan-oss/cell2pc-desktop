...
///
/// A modem is on before you sit down and stays on after you log out. Cell2Pc
...
{
    private const string ServiceName = "Cell2Pc";
    ...
    public static int Install(CommandLine cmd)
    {
        ...
        string exe = Environment.ProcessPath
            ?? Path.Combine(AppContext.BaseDirectory, "cell2pc.exe");
        ...
        Output.Info("");
        Output.Hint("Remove it later with:  cell2pc uninstall-service");
        return 0;
    }
    ...
    private static int InstallSystemd(CommandLine cmd)
    {
        ...
        string exe = Environment.ProcessPath ?? "/usr/local/bin/cell2pc";
        string unit = $"""
        [Unit]
        Description=Cell2Pc - shares a phone's mobile data with this PC
        ...
        try
        {
            ...
            // world-readable.
            const string path = "/etc/systemd/system/cell2pc.service";
            ...
            Run("systemctl", "enable cell2pc");
            Run("systemctl", "start cell2pc");

            Output.Ok("installed and started");
            Output.Hint("Check it with:  systemctl status cell2pc");
            return 0;
        }
        catch (Exception ex)
        {
    ...
    {
        Run("systemctl", "stop cell2pc");
        Run("systemctl", "disable cell2pc");
        try { File.Delete("/etc/systemd/system/cell2pc.service"); } catch { }
        Run("systemctl", "daemon-reload");
