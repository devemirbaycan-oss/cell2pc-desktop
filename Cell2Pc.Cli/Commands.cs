...
internal static class Commands
{
    ...
    public static async Task<int> ConnectAsync(CommandLine cmd)
    {
        ...
        {
            Output.Error("Cell2Pc supports Windows and Linux.");
            return 1;
        }
        ...
        if (!PlatformFactory.IsJoinedToPhone())
        {
            ...
            if (string.IsNullOrWhiteSpace(passphrase))
            {
                if (Console.IsInputRedirected)
                {
                    ...
                    Output.Hint("Pass --passphrase, or set CELL2PC_PASSPHRASE.");
                    return 1;
                }
            ...
            if (!await joiner.JoinAsync(passphrase))
            {
                ...
                Output.Hint("Run 'cell2pc doctor' for a fuller check.");
                return 1;
            }
            Output.Ok("joined");
        }
        ...
        if (!await session.StartAsync(stopped.Token))
        {
            ...
            Output.Hint("Run 'cell2pc doctor' to find out why.");
            joiner?.Leave();
            return 1;
        }
    ...
    public static int Status(CommandLine cmd)
    {
        ...
        else if (routed && !adapterUp)
        {
            ...
            Output.Hint("This is the state a crash leaves. Run: cell2pc recover");
        }
    ...
    public static async Task<int> HandoverAsync(CommandLine cmd)
    {
        ...
        var running = System.Diagnostics.Process.GetProcessesByName("Cell2Pc-Desktop")
            .Concat(System.Diagnostics.Process.GetProcessesByName("cell2pc"))
        ...
        if (!await session.StartAsync(stopped.Token))
        {
            ...
            Output.Hint("Run 'cell2pc recover' if the machine has no internet.");
            return 1;
        }
    ...
    public static async Task<int> DoctorAsync(CommandLine cmd)
    {
        ...
        else
        {
            ...
            if (joiner.IsVisible())
                problems.Add("The phone's group is visible. Connect to it, or run 'cell2pc connect' with the passphrase.");
            else
        ...
        if (HasTunnelRoutes() && FindAdapter() is null)
        {
            ...
            problems.Add("Run 'cell2pc recover' to restore normal routing.");
        }
    ...
    private static string? ResolveToken(CommandLine cmd)
    {
        ...
        if (Console.IsInputRedirected)
        {
            ...
            Output.Hint("Pass --token, or set CELL2PC_TOKEN.");
            return null;
        }
    ...
    private static NetworkInterface? FindAdapter() =>
        NetworkInterface.GetAllNetworkInterfaces()
            .FirstOrDefault(n => n.Name.Contains("cell2pc", StringComparison.OrdinalIgnoreCase));
