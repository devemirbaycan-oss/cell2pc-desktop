...
internal sealed class CommandLine
{
    ...
    public static CommandLine Parse(string[] args)
    {
        ...
        string? token = Environment.GetEnvironmentVariable("POCKETMODEM_TOKEN");
        string? passphrase = Environment.GetEnvironmentVariable("POCKETMODEM_PASSPHRASE");
        ...
        // A bare address is a convenience: `pocketmodem 192.168.49.1`.
    ...
    public static void PrintHelp(string? verb = null)
    {
        ...
        Console.WriteLine($"""
        pocketmodem {BuildInfo.Version} - your PC on the phone's mobile data

        USAGE
          pocketmodem [command] [options]
        ...
        ENVIRONMENT
          POCKETMODEM_TOKEN       Pairing code, so it need not be typed
          POCKETMODEM_PASSPHRASE  Wi-Fi passphrase
          NO_COLOR                 Disable colour

        EXAMPLES
          pocketmodem                          Connect, prompting for the code
          pocketmodem -t k7m2xq4p               Connect with a known code
          pocketmodem connect --for 2h          Connect for two hours
          pocketmodem doctor                    Work out why it will not connect
          pocketmodem recover                   Fix routing after a crash
    ...
    {
        Console.WriteLine("""
        pocketmodem connect - route all PC traffic through the phone

        USAGE
          pocketmodem connect [options]
